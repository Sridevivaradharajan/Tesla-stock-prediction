    ...
    if page == "Prediction":
        ...
        if uploaded_file is not None:
            try:
                ...
                # Clean and convert numeric columns - FIXED VERSION
                ...
                for col in numeric_columns:
                    if col in df.columns:
                        if df[col].dtype == 'object':
                            dollar_sign = '$'
                            comma_sign = ','
                            df[col] = df[col].astype(str).str.replace(dollar_sign, '', regex=False)
                            df[col] = df[col].str.replace(comma_sign, '', regex=False)
